...
# WSL Compatibility     | Yes (Windows Subsystem for Linux)
...
# Snap Docker can only access /home, /media, /mnt, /tmp, and /var/snap
detect_snap_docker() {
  if snap list docker &>/dev/null || command -v snap &>/dev/null && snap list 2>/dev/null | grep -q docker; then
    return 0  # Docker is installed via snap
  fi
  return 1  # Docker is not installed via snap
}

# Detect if running under Windows Subsystem for Linux
detect_wsl() {
  if grep -qiE "(Microsoft|WSL)" /proc/version 2>/dev/null; then
    return 0  # Running under WSL
  fi
  return 1  # Not running under WSL
}
...
# Set installation directory based on Docker installation method
if detect_snap_docker; then
  # Use user's home directory for snap Docker (snap can access /home)
  NOMAD_DIR="${HOME}/project-nomad"
else
  # Use standard location for native Linux
...
ensure_docker_installed() {
  if ! command -v docker &> /dev/null; then
    ...
    # Check if Docker service is running (or accessible via WSL integration)
    if ! systemctl is-active --quiet docker 2>/dev/null; then
      # Try starting Docker (works on native Linux)
      # On WSL2 with Docker Desktop, this may not work, but Docker may still be available
      if ! docker info &> /dev/null; then
        # Docker command exists but daemon is not accessible
        echo -e "${YELLOW}#${RESET} Docker daemon is not accessible. Starting Docker...\\n"
        sudo systemctl start docker 2>/dev/null || true
        sleep 2
        if ! docker info &> /dev/null; then
          echo -e "${RED}#${RESET} Failed to access Docker daemon. Please ensure Docker is running."
          exit 1
        fi
      fi
      echo -e "${GREEN}#${RESET} Docker service accessible.\\n"
    else
...
create_nomad_directory(){
  ...
  if [[ ! -d "$NOMAD_DIR" ]]; then
    ...
    # Use sudo only for system directories like /opt
    if [[ "$NOMAD_DIR" == /opt/* ]]; then
...
download_management_compose_file() {
  ...
  # Replace hardcoded /opt/project-nomad paths with the dynamic NOMAD_DIR
  # This is needed for snap Docker which cannot access /opt due to filesystem confinement
  if [[ "$NOMAD_DIR" != "/opt/project-nomad" ]]; then
    echo -e "${YELLOW}#${RESET} Updating volume paths for compatibility...\\n"
    sed -i "s|/opt/project-nomad|${NOMAD_DIR}|g" "$compose_file_path"
  fi
  
  # Fix mount propagation for WSL - rslave is not supported on WSL
  if [[ "$IS_WSL" == "true" ]]; then
    echo -e "${YELLOW}#${RESET} Adjusting mount propagation for WSL compatibility...\\n"
    sed -i "s|:/host:ro,rslave|:/host:ro|g" "$compose_file_path"
  fi
  
  # For snap Docker, healthchecks that use exec (redis-cli, mysqladmin) fail
  # because of snap's mount namespace restrictions. Remove healthcheck conditions.
  if detect_snap_docker; then
    echo -e "${YELLOW}#${RESET} Removing healthcheck conditions for snap Docker compatibility...\\n"
    # Remove healthcheck conditions from depends_on (change service_healthy to service_started)
    sed -i 's/condition: service_healthy/condition: service_started/g' "$compose_file_path"
    # Optionally remove the healthcheck blocks entirely
    sed -i '/healthcheck:/,/retries: 3/d' "$compose_file_path"
  fi
